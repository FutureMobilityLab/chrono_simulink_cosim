    ...
    try {
        ...
        while (vis->Run()) {
            ...
            while (my_time < sim_time) {
                ...
                car.GetSuspension(0)->GetAxle(LEFT)->SetAppliedLoad(-data_in(1));
                car.GetSuspension(0)->GetAxle(RIGHT)->SetAppliedLoad(-data_in(2));
                car.GetSuspension(1)->GetAxle(LEFT)->SetAppliedLoad(-data_in(3));
                car.GetSuspension(1)->GetAxle(RIGHT)->SetAppliedLoad(-data_in(4));

                // Override individual brake actuation.
                car.GetBrake(0, LEFT)->Synchronize(my_time, data_in(5));
                car.GetBrake(0, RIGHT)->Synchronize(my_time, data_in(6));
                car.GetBrake(1, LEFT)->Synchronize(my_time, data_in(7));
                car.GetBrake(1, RIGHT)->Synchronize(my_time, data_in(8));
            ...
            ChVector3<> wheel_normal = car.GetWheel(0,LEFT)->GetState().rot.GetAxisY();
            ...
            wheel_normal = car.GetWheel(0,RIGHT)->GetState().rot.GetAxisY();
            normal = car.GetChassis()->GetTransform().TransformDirectionParentToLocal(wheel_normal);
            data_out(52) = std::atan2(normal.x(), normal.y());
            wheel_normal = car.GetWheel(1,LEFT)->GetState().rot.GetAxisY();
            normal = car.GetChassis()->GetTransform().TransformDirectionParentToLocal(wheel_normal);
            data_out(53) = std::atan2(normal.x(),normal.y());
            wheel_normal = car.GetWheel(1,RIGHT)->GetState().rot.GetAxisY();
            normal = car.GetChassis()->GetTransform().TransformDirectionParentToLocal(wheel_normal);
