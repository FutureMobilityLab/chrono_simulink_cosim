...
namespace chrono::vehicle
{
  ...
  // -----------------------------------------------------------------------------
  // -----------------------------------------------------------------------------
  WheeledVehicleForce::WheeledVehicleForce(const std::string &filename,
  ...
  WheeledVehicleForce::WheeledVehicleForce(ChSystem *system, const std::string &filename, bool create_powertrain, bool create_tires)
  ...
  // -----------------------------------------------------------------------------
  // -----------------------------------------------------------------------------
  void WheeledVehicleForce::Create(const std::string &filename, bool create_powertrain, bool create_tires)
  {
    ...
    assert(subtype.compare("WheeledVehicleForce") == 0);
    ...
    for (int i = 0; i < m_num_strs; i++)
    {
      ...
      m_steerings[i] = ReadForceSteeringJSON(vehicle::GetDataFile(file_name));
    ...
    if (d.HasMember("Driveline"))
    {
      ...
      rapidjson::SizeType num_driven_axles = d["Driveline"]["Suspension Indexes"].Size();
      m_driven_axles.resize(num_driven_axles);
      for (rapidjson::SizeType i = 0; i < num_driven_axles; i++)
      {
    ...
    std::cout << "Loaded JSON: " << filename.c_str() << "\n";
  }

  // -----------------------------------------------------------------------------
  // -----------------------------------------------------------------------------
  void WheeledVehicleForce::Initialize(const ChCoordsys<> &chassisPos, double chassisFwdVel)
  {
