...
SalaaniTireModel::Forces SalaaniTireModel::calculateTireForces(double ALPHA, double S,
                                                             double GAMMA, double FZ) {
  // Cap vertical force to valid range.
  double Fz = std::min(tire_params.FzMax, FZ);
  ...
  double MURATIO = tire_params.MUNOM / tire_params.MUNTEST;

  // Calculate empirical physical properties

  // Pneumatic trail (Equation 29)
  double Tz = tire_params.tz1 * FZ2 + tire_params.tz2 * Fz;
  
  // Overturning moment arm (Equation 30)
  double Tx = tire_params.tx1 * FZ2 + tire_params.tx2 * Fz + tire_params.tx3;
    
  // Lateral stiffness (Equation 26)
  double CA = tire_params.Cam * (1.0 - std::exp(tire_params.C1 * std::pow(Fz/tire_params.FZCam, 2) + 
                                          tire_params.C2 * (Fz/tire_params.FZCam)));
  
  // Longitudinal stiffness (Equation 27)
  double CS = tire_params.Ckm * std::pow(Fz/tire_params.FZCKM, tire_params.n_val);
  
  // Inclination angle lateral force stiffness (Equation 31)
  double FYGAMMA = tire_params.Cr1 * Fz + tire_params.Cr2 * FZ2;
  
  // Determine reference load for friction calculations
  double FZ1 = (Fz < tire_params.FZ0) ? tire_params.FZ0 : Fz;
  
  // Longitudinal peak coefficient of friction (Equation 22)
  double MUXp = MURATIO * tire_params.mu_p0_long * 
                std::pow(FZ1/tire_params.FZ0, tire_params.eta0_long + tire_params.eta1_long * std::log(FZ1/tire_params.FZ0));
  ...
  double DMUx = tire_params.d1_long * FZ2 + tire_params.d2_long * Fz + tire_params.d3_long;
  
  // Lateral decay of friction (Equation 23)
  double DMUy = tire_params.d1_lat * FZ2 + tire_params.d2_lat * Fz + tire_params.d3_lat;
  ...
  double SIGMA = std::sqrt(std::pow(CA * std::tan(alpha_mod) / (1.0-S_clamped) / MUYp / Fz, 2) +
                          std::pow(CS * S_clamped / (1.0-S_clamped) / MUXp / Fz, 2));
  ...
  SalaaniTireModel::Forces forces;
  ...
  if (SIGMAm < 1.0e-6) {
      ...
      forces.FY = Fz * CA * std::tan(alpha_mod) * (F_a/SIGMAm + F_s/SIGMASm);
      forces.FX = -Fz * S_clamped * (CS * F_a/SIGMAm + CSp * F_s/SIGMASm);
  }
