...
int main() {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        auto forces = tire.calculate(fz_test, alpha, 0.0, 0.0);
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        auto forces = tire.calculate(fz_test, 0.0, slip, 0.0);
    ...
    // Header will be: Slip_Set_ID, Slip_Value, Alpha_deg, Fx, Fy
    for (size_t s_idx = 0; s_idx < slip_conditions.size(); ++s_idx) {
        ...
        for (int i = 0; i < 60; ++i) {
            ...
            auto forces = tire.calculate(fz_test, alpha, s, 0.0);
